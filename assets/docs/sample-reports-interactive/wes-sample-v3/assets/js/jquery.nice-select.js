    ...
    (function($) {
      ...
      $.fn.niceSelect = function(method) {
        ...
        $(document).on('click.nice_select', '.nice-select', function(event) {
	        ...
	        //2022.03.24 select option resize width 
	        var $dropdownListText = $dropdownList.text().split(" ");
	        var $textpreLength =0;
	        for(var i in $dropdownListText){
	        	var textLen = $dropdownListText[i];
	        	var text = $dropdownListText[i];
	        	if($textpreLength == 0 ){
	        		$textpreLength =textLen.length;        		
	        	}else{
	        		if($textpreLength < textLen.length){
	        			$textpreLength =textLen.length;       
	        		}
	        	}
	        }
	        ...
	        if($textpreLength > 10){
	        	...
	        	widthResize = $dropdown.width()+$textpreLength;
	        }
	        ...
	        var posLeft = $dropdown.offset().left +1;
	          ...
	          $('.selectList[data-num]').not($dropdownList).removeClass('open');;
	            ...
	            setTimeout(function(){
	              ...
	              /*$dropdownList.css({top:posTop,left:posLeft, 'min-width':$dropdown.outerWidth()-2})*/
	              $dropdownList.css({top:posTop,left:posLeft, 'min-width':$dropdown.outerWidth()-2, 'width': widthResize+"px"})
	            }, 100)
        ...
        $(document).on('click.selectList', '.selectList .option:not(.disabled)', function(event) {
          ...
          $dropdown.prev('select').val($option.data('value')).trigger('change');
	          ...
	          var $dropdownId = $dropdown.data('id');
	    	  
	          console.log("$dropdownNum : "+$dropdownNum+", $dropdown : "+$dropdown+", $dropdownId : "+$dropdownId+", $option.data : "+$option.data('value'));
	          
	    	  if($dropdownId == "topSelectBox"){
	          	var optVal=$option.data('value');	          	
	          	$("#"+optVal).click();
	    	  }
