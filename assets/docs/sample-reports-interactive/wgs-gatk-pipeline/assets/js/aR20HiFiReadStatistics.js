...
//	'</span></div></div><div class="w3-col l7"><span style="font-size: 12px; color: #9e9e9e;">HiFi Read Bases</span><br>',
//	'<span style="font-size: 34px; color: #00a8a6;">',
//	'</span></div></div></div><div class="w3-col l6"><div class="w3-row"><div class="w3-col l3"><span style="font-size: 12px; color: #9e9e9e;">HiFi Reads</span><br>',
...
//	'<td><span style="font-size: 16px; font-weight: bold; line-height: 1.88; color: #757575;">&nbsp;&nbsp;HiFi Read Length Distribution</span></td>',
//	'</tr></table><canvas id="',
...
//	'<td><span style="font-size: 16px; font-weight: bold; line-height: 1.88; color: #757575;">HiFi Read Quality Distribution</span></td>',
//	'</tr></table><canvas id="',
...
(function () {
  'use strict'
	...
	$('#quick_menu_pages_ul').find("a[href*=aR20HiFiReadStatistics]").css("font-weight", "bold");
	
	
	var statisticsDataAll = new Array();
	var statisticsDataRow;
	for(var idx in hifi_read_statistics.data){
		statisticsDataAll.push({
			"sdata":hifi_read_statistics.data[idx],
			"lengthChart":hifi_read_length_distribution_chart_list[idx],
			"qualityChart":hifi_read_quality_distribution_chart_list[idx]
		});
	}
	...
	for(var idx in statisticsDataAll){
		...
		for(var subidx in statisticsDataAll[idx].lengthChart.data){
			data2Label = statisticsDataAll[idx].lengthChart.data[subidx].label;
...
function readLengthChart(canvasID, labelArr, data1Label, data2Label, data1Arr, data2Arr){
	    ...
	    options: {
	    	...
	    	scales: {
	    		y:{ 
	    			...
	    			//title:{display:true, text:data1Label} 
    			},
    			x:{ 
