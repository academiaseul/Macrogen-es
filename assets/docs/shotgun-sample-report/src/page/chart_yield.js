...
//zoom 비율 설정
if (array_yield.length < 7) {
	zoom_rat = 1;
} else {
	zoom_rat = 1 * array_yield.length / 7;
}
...
var yAxis_yield = d3.axisLeft(y_yield).ticks(7).tickFormat(d3.format(",.0s"));
...
state_yield.selectAll("rect")
	...
	.attr("height", function(d){return height_yield - y_yield(d.Value);
	});

//그래프 상단 텍스트
state_yield.selectAll("text")
	.data(function(d) {
		return d.Categories;
	})
	.enter().append("text")
...
var legend = LegendHolder.selectAll(".legend_yield")
	...
	.each(function(d, i) {
		...
		d3.select(this).append("text")
			...
			.text(d.Name);
		textWidthHolder_yield += getTextWidth(d.Name, "10px", "calibri");
	});
